...
# class OtpJwtAuthBackends(ModelBackend):
#     """
#     Login user using sms/email(without password) and setup jwt
#     """
#
#     def authenticate(self, request, username=None):
#         method = 'sms' if re.search('^[0-9]{10,16}$') else 'email'
#         try:
#             user = (User.objects.get(phoneNumber=username) if method == 'sms' else
#                     User.objects.get(email=username))
#         except User.DoesNotExist:
#             user = (User(phoneNumber=username) if method == 'sms' else
#                     User(email=username))
