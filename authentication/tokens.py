...
class Token:
    ...
    def __init__(self, token=None, verify=True):
        ...
        if token is not None:
            ...
            self.payload = {settings.AUTHENTICATION['TOKEN_TYPE_CLAIM']: self.token_type}
    ...
    def verify(self):
        ...
        if settings.AUTHENTICATION['JTI_CLAIM'] not in self.payload:
    ...
    def verify_token_type(self):
        ...
        """
        try:
            token_type = self.payload[settings.AUTHENTICATION['TOKEN_TYPE_CLAIM']]
        except KeyError:
    ...
    def set_jti(self):
        ...
        """
        self.payload[settings.AUTHENTICATION['JTI_CLAIM']] = uuid4().hex
    ...
    def for_user(cls, user):
        ...
        """
        user_id = getattr(user, settings.AUTHENTICATION['USER_ID_FIELD'])
        if not isinstance(user_id, int):
            user_id = str(user_id)

        token = cls()
        token[settings.AUTHENTICATION['USER_ID_CLAIM']] = user_id
...
class AccessToken(Token):
    token_type = 'access'
    lifetime = settings.AUTHENTICATION['ACCESS_TOKEN_LIFETIME']