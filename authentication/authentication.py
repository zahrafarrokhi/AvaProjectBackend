...
AUTH_HEADER_TYPES = settings.AUTHENTICATION['AUTH_HEADER_TYPES']

if not isinstance(settings.AUTHENTICATION['AUTH_HEADER_TYPES'], (list, tuple)):
...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def get_header(self, request):
        ...
        header = request.META.get(
            settings.AUTHENTICATION['AUTH_HEADER_NAME'])
    ...
    def get_validated_token(self, raw_token):
        ...
        for AuthToken in settings.AUTHNEENTICATION['AUTH_TOKEN_CLASSES']:
            try:
    ...
    def get_user(self, validated_token):
        ...
        """
        try:
            user_id = validated_token[settings.AUTHNEENTICATION['USER_ID_CLAIM']]
        except KeyError:
            raise InvalidToken(
                _('Token contianed no recognizable user identification'))
        try:
            user = self.user_model.objects.get(**{settings.AUTHNEENTICATION['USER_ID_FIELD']:
                                                  user_id})
...
class JWTTOkenUserAuthentication(JWTAuthentication):
    def get_user(self, validated_token):
        ...
        """
        if settings.AUTHNEENTICATION['USER_ID_CLAIM'] not in validated_token:
            raise InvalidToken(
                _('Token contained no recognizable user identification'))

        return settings.AUTHNEENTICATION['TOKEN_USER_CLASS'](validated_token)
